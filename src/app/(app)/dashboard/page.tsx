...
import { subscribeToClients, subscribeToInvoices, subscribeToTasks, subscribeToInventory } from "@/lib/firestore";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
        new Promise(resolve => onSnapshot(query(collection(db, 'clients')), () => resolve(true))),
        new Promise(resolve => onSnapshot(query(collection(db, 'invoices')), () => resolve(true))),
        new Promise(resolve => onSnapshot(query(collection(db, 'tasks')), () => resolve(true))),
        new Promise(resolve => onSnapshot(query(collection(db, 'inventory')), () => resolve(true))),
    ]).then(() => {
        setLoading(false);
    });
  ...
  const activeInvoices = invoices.filter(inv => inv.status === 'Unpaid' || inv.status === 'Overdue').length;
    ...
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {loading ? (
          ...
          <>
            ...
            <StatCard title="Active Invoices" value={activeInvoices.toString()} icon={FileText} />
            <StatCard title="Pending Tasks" value={pendingTasks.toString()} icon={CheckCircle2} />
            <StatCard title="Inventory Items" value={inventory.length.toString()} icon={Boxes} />
          </>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card>
          ...
          <CardContent>
             {loading ? <Skeleton className="h-48 w-full" /> : recentInvoices.length > 0 ? (
            <ScrollArea className="h-64">
              <Table>
                ...
                <div className="flex h-48 items-center justify-center text-center text-muted-foreground">
                    <p>No invoices found. <br/> Create your first one to get started.</p>
                </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            {loading ? <Skeleton className="h-48 w-full" /> : recentTasks.length > 0 ? (
            <ScrollArea className="h-64">
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <div className="flex h-48 items-center justify-center text-center text-muted-foreground">
                    <p>No pending tasks. <br /> Enjoy your day!</p>
                </div>
            )}
          </CardContent>
        </Card>
